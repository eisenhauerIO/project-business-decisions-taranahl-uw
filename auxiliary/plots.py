...
def plot_probability_path(trade_df, ticker="", ax=None):
    ...
    ax.scatter(df["datetime"], df["probability"] * 100,
               alpha=0.3, s=8, color=BLUE, label="Trades")

    if len(df) >= 10:
        smoothed = df["probability"].rolling(20, min_periods=1).median() * 100
        ax.plot(df["datetime"], smoothed, color=RED, linewidth=2,
...
def plot_fan_chart(sim_paths, actual_path=None, p0=None, title="",
    ...
    ax.fill_between(t, q_lo_outer, q_hi_outer, alpha=0.15, color=BLUE,
                    label=f"{int(quantiles[0]*100)}–{int(quantiles[3]*100)}th pctile")
    ax.fill_between(t, q_lo_inner, q_hi_inner, alpha=0.30, color=BLUE,
    ...
    if actual_path is not None:
        ...
        ax.plot(t_actual, actual_path * 100, color=RED, linewidth=2.5,
                zorder=5, label="Observed path")
    ...
    if p0 is not None:
        ax.annotate(f"Open: {p0*100:.0f}%", xy=(0, p0 * 100),
                    xytext=(5, p0 * 100 + 5), fontsize=9, color=GRAY)
...
def plot_opening_distribution(markets_df, series_label="NBA Game Winner"):
    ...
    ax.set_title(
        f"Distribution of Market Probabilities — {series_label}", fontsize=12
    )
    ax.legend()
    plt.tight_layout()
    return fig
...
def plot_calibration(cal_df, title="Market Calibration"):
    ...
    ax.scatter(valid["predicted_prob"], valid["actual_win_rate"],
               s=counts * 0.5 + 30,
               color=BLUE, alpha=0.8, edgecolors="white", linewidths=0.5,
               label="Observed")
...
def plot_volatility_term_structure(histories, n_steps=50):
    ...
    for df in histories.values():
        ...
        prep["bin"] = pd.cut(
            prep["time_pct"], bins=n_steps,
            labels=range(n_steps), include_lowest=True
        )
    ...
    ax.scatter(x, y, s=[s * 0.3 + 10 for s in sizes],
